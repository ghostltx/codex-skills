...
function preserveUserMediaDirs(outDir) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xeliasai-user-media-'));
  const entries = [
...
// `XELIASAI_PPT_THEME_RUNTIME`:auto(默认,有源走源、无源走预构建)| prebuilt(强制预构建,测试用)
...
function buildImportedThemeRuntime(outFile, usedThemeKeys = []) {
  ...
  const mode = process.env.XELIASAI_PPT_THEME_RUNTIME || 'auto';
...
function resolveThemeRegistryEntry(usedThemeKeys) {
  ...
  const cacheDir = path.join(ROOT, 'node_modules/.cache/xeliasai-theme-registry');
