...
//
// 单一事实来源:client-runtime.jsx 经 `@xeliasai/theme-registry` 别名引入注册表。两条等价路径:
...
function clientRuntimeBuildOptions({ root, outFile, registryPath }) {
  return {
    ...
    jsx: 'automatic',
    alias: {
      '@xeliasai/theme-registry': registryPath,
...
export function buildThemeModule({ root, themeKey, outDir }) {
  ...
  const cacheDir = path.join(root, 'node_modules/.cache/xeliasai-theme-runtime-build');
...
function writeTempRegistry(root, source, cacheDirOverride) {
  const cacheDir = cacheDirOverride || path.join(root, 'node_modules/.cache/xeliasai-theme-registry');
  fs.mkdirSync(cacheDir, { recursive: true });
