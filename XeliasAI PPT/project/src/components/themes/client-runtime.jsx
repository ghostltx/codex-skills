...
// renderDeck 打包时把 `@xeliasai/theme-registry` 别名指向「全主题」或「按 deck 实际用到的主题裁剪版」。
import { runtimePages, wrapThemeImageProviders } from '@xeliasai/theme-registry';
...
function renderMedia(value, props = {}) {
  ...
  if (item.kind === 'video') {
    ...
    if (!active) {
      return poster
        ? <img src={poster} alt="" loading="lazy" decoding="async" data-xeliasai-video-poster="true" {...mediaProps} />
        : <div aria-hidden="true" data-xeliasai-video-poster="true" {...mediaProps} />;
    }
    return (
      <video
        src={item.src}
...
function HostImageSlot({ mediaApi, index, options = {} }) {
  ...
  return (
    <div
      data-xeliasai-host-image-slot="true"
      data-xeliasai-media-kind={item?.kind || ''}
      data-xeliasai-video-native={preserveVideoSize ? 'true' : undefined}
      data-xeliasai-video-ratio={preserveVideoRatio ? 'true' : undefined}
      data-xeliasai-contain-media={containMedia ? 'true' : undefined}
      data-xeliasai-adaptive-media={adaptiveMedia ? 'true' : undefined}
      data-xeliasai-fallback-ratio={adaptiveMedia && fallbackRatio ? String(fallbackRatio) : undefined}
      style={{
        position: 'relative',
...
function bindRenderedImageSlots(root, mediaApi) {
  ...
  root.addEventListener('dragover', event => {
    ...
    event.preventDefault();
    slot.classList.add('is-xeliasai-drag-over');
  });

  root.addEventListener('dragleave', event => {
    const slot = event.target.closest?.('.gxn-slot');
    if (slot && root.contains(slot)) slot.classList.remove('is-xeliasai-drag-over');
  });
  ...
  root.addEventListener('drop', event => {
    ...
    if (!slot || !root.contains(slot)) return;
    event.preventDefault();
    slot.classList.remove('is-xeliasai-drag-over');
  ...
  // notifies content changes via a bubbling 'xeliasai-image-slot-change' event
  ...
  // control. The fallback element can declare a `data-xeliasai-media-extra-props`
  ...
  root.addEventListener('xeliasai-image-slot-change', event => {
    ...
    const extraProps = value ? readJson(slot.getAttribute('data-xeliasai-media-extra-props'), null) : null;
...
function normalizeExternalValues(entry, defaults, values) {
  ...
  if (error) {
    console.warn(`[xeliasai-ppt] dropped invalid prop(s) for "${entry.key}" instead of crashing the render`, error);
  }
...
function renderRuntimeThemeSlide(slide, values = {}, options = {}) {
  ...
  try {
    ...
    if (pageSpec) root.dataset.xeliasaiDynamicPageProps = pageSpec.kind;
    window.__syncDeckPageNumbers?.(slide);
    return true;
  } catch (error) {
    console.error(`[xeliasai-ppt] runtime render failed for "${root.dataset.pageKey || entry.key}"`, error);
    return false;
  }
}
