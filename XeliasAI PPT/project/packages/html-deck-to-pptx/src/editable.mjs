...
export async function exportEditablePptxFromPage(page, options = {}) {
  ...
  pptx.defineLayout({ name: 'XELIASAI_WIDE', width: PPT_W, height: PPT_H });
  pptx.layout = 'XELIASAI_WIDE';
  pptx.author = 'XeliasAI PPT';
...
async function collectEditableDeck(page, options = {}) {
  await page.evaluate(async ({ includeAllThemePacks }) => {
    ...
    if (!document.getElementById('xeliasai-export-no-radius')) {
      const style = document.createElement('style');
      style.id = 'xeliasai-export-no-radius';
...
function shouldScreenshotRoundedVisual(el, style, clipped, rawRect, slideRect) {
  ...
  return Boolean(el.querySelector?.('image-slot,[data-xeliasai-host-image-slot="true"],svg,canvas,img,video'))
...
function isImageSlotElement(el) {
  return !!el.matches?.('image-slot,[data-xeliasai-host-image-slot="true"]');
}
