...
// XELIASAI_PPT_PREVIEW_IDLE_MS 是仅测试用的毫秒级后门,优先于 XELIASAI_PPT_PREVIEW_IDLE_HOURS。
// XELIASAI_PPT_PREVIEW_IDLE_CHECK_MS 同样仅测试用,覆盖检查间隔以加速回归测试。
...
function resolveIdleLimitMs() {
  const msOverride = Number(process.env.XELIASAI_PPT_PREVIEW_IDLE_MS);
  if (Number.isFinite(msOverride) && msOverride >= 0) return msOverride;
  const hoursRaw = process.env.XELIASAI_PPT_PREVIEW_IDLE_HOURS;
...
function resolveIdleCheckIntervalMs(idleLimitMs) {
  const override = Number(process.env.XELIASAI_PPT_PREVIEW_IDLE_CHECK_MS);
