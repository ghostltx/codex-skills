...
const requestedPort = Number(process.env.XELIASAI_PPT_PREVIEW_PORT || process.argv[3] || 4178);
const host = process.env.XELIASAI_PPT_PREVIEW_HOST || process.env.HOST || '0.0.0.0';
const localName = process.env.XELIASAI_PPT_PREVIEW_NAME || os.hostname().split('.')[0] || 'localhost';
const portScanLimit = Math.max(40, Number(process.env.XELIASAI_PPT_PREVIEW_PORT_SCAN || 240));
const lockDir = process.env.XELIASAI_PPT_PREVIEW_LOCK_DIR || path.join(os.tmpdir(), 'xeliasai-ppt-preview-ports');
