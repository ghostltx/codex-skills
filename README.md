...
`v1.43` 将一批已在本机使用的个人技能纳入版本库，并更新 `comfyui-scene-split` 与两套 Banana 图像生成入口：

- 新增搜索、品牌、设计系统、详情页、PSD、宠物、批量水印、夸克网盘、合成表演者标签和 UI/UX 相关技能。
- 新增 `rh100-banana-pro-us/` 与 `rh100-t2i-i2i-banana2-us/`，便于跨设备使用两套 Banana 图像生成工作流。
- 更新 `comfyui-scene-split/`，保持场景拆分工作流与当前本机版本一致。
- 排除构建产物、Python 缓存、嵌套 Git 元数据及夸克网盘的设备认证与搜索运行数据，仓库只保存可复用的技能源码和说明。

简而言之，`v1.43` 扩大了个人 Skills 的跨设备同步范围，同时保持仓库内容可复用、可维护。

### v1.42

`v1.42` 删除已弃用的 `rh100-t2i-i2i/`（中国站标准版 RunningHub G-2）技能及其提交、批处理、下载脚本与 API 说明：

- 移除重复且当前不再使用的标准版入口，避免与 `rh100-t2i-i2i_US/` 混淆。
- 后续若需要 RunningHub US GPT-Image-2.5 生图或编辑，统一使用 `rh100-t2i-i2i_US/`。
- 删除仅影响该独立技能目录；其他个人技能保持不变。

简而言之，`v1.42` 清理废弃的标准版 RH100 入口，使技能列表更明确。

### v1.41

`v1.41` 新增并发布 `comfyui-scene-split/`，用于把带人物和桌椅的场景图拆成可在 ComfyUI 重新合成的素材：

- 自动识别人物；有人时输出保持动作、构图与画幅的白底人物图，无人时跳过人物图。
- 生成去除人物、桌椅、文字和标识后的自然补全背景图，并保留原场景光线、透视和环境细节。
- 同时保存一段英文全场景提示词，完整记录人物、环境和桌椅特征，方便分别渲染家具后合成。
- 每次任务都在桌面按原图文件名创建文件夹，保存人物图、背景图和提示词 TXT，避免与其他素材混淆。

简而言之，`v1.41` 为 ComfyUI 场景重组提供了人物、背景和家具提示词三份对齐素材。

### v1.40

`v1.40` 升级 `rh100-t2i-i2i_US/` 至 RunningHub US GPT-Image-2.5 官方 Token 三模式入口：

- 文生图固定使用 Sunburst 文生图路由。
- 图生图默认使用 Sunburst 高精度编辑；支持最多 16 张参考图、背景、质量与输出格式参数。
- 新增 Flare 高速图生图，但只有用户明确要求 “Flare” 或“高速图生图”时才会启用，避免默认牺牲精修优先级。
- 批处理脚本、API 参考、界面默认提示和离线模拟测试同步更新；保留提交后确认下载的既有流程。

简而言之，`v1.40` 让 US 图像 skill 同时提供文生图、默认高精度图生图和按需高速图生图三种明确入口。

### v1.39

`v1.39` 新增并发布 `rh100-t2i-i2i_US/`，补充 RunningHub US 站的 GPT-Image-2 文生图与图生图调用入口：

- 新增 US 站 RH100 G-2 文生图、图生图、批量提交和结果下载脚本。
- US 版调用 `runninghub.ai` 接口，使用 `RUNNINGHUB_API_KEY_US`，与中国站标准版的 `RUNNINGHUB_API_KEY` 分开配置。
- 保持与标准版一致的自动路由规则：无参考图走文生图，提供本地图片或图片 URL 时走图生图。
- 保留提交后先确认、确认后再轮询和下载全部结果的工作流。
- 标准版 `rh100-t2i-i2i/` 继续调用中国站接口；两个版本均面向 RunningHub GPT-Image-2/G-2 低价通道。
### v1.38

`v1.38` 更新了 `rh100-t2i-i2i/` 的提交后交互和下载能力：

- 文生图与图生图提交后默认只输出 RunningHub 任务页链接，不再自动弹出浏览器。
- 保留 `--open` 作为用户明确要求时的可选浏览器打开参数，并兼容旧的 `--no-open` 参数。
- 新增文生图多变体批处理脚本，减少长 PowerShell 命令导致的执行拦截。
- 下载脚本新增 `--output-dir`，支持将全部结果直接保存到用户指定文件夹。
- 同步更新 G-2 文生图/图生图接口、任务确认和批量提交说明。
...
- 删除 `RH-GPT-IMAGE-2-I2I/`，不再保留独立的旧图生图提交脚本和接口说明。
- 删除 `RH-GPT-IMAGE-2-T2I/`，不再保留独立的旧文生图生成、查询和下载脚本。
- 从 `.gitignore` 白名单移除这两个目录，避免它们以后被误加入仓库。
- 后续文生图和图生图统一使用 `rh100-t2i-i2i/`，由一个入口自动选择 T2I 或 I2I。

简而言之，`v1.35` 删除两套重复旧入口，仓库只保留当前统一的 RH100 图片生成流程。

### v1.34

`v1.34` 将重复的 RunningHub 图片生成入口整合为 `RH100-T2I/I2I`，统一文生图、图生图、提交确认和结果下载流程：

- 新增 `rh100-t2i-i2i/` 作为唯一 RH100 图片生成 skill；内部名称遵循小写连字符规范，界面显示为 `RH100-T2I/I2I`。
- 未提供参考图片时自动调用 RH100-T2I，提供本地图片或图片 URL 时自动调用 RH100-I2I。
- 提交成功后打开 RunningHub 任务页面，但不立即轮询；先询问用户是否下载全部图片。
- 用户确认下载后，轮询本次全部任务，并以第一个 `taskId` 在桌面创建文件夹，下载所有任务返回的全部图片。
- 新增 `scripts/rh100_download.py`，支持单个或多个 `taskId`，也支持从批量任务的 `rh100_jobs.json` 恢复下载。
- 删除重复或过时的 `RH-API/`、`RH100/`、`文生图/` 和 `图生图/` 仓库入口，减少调用时的路由混淆。

简而言之，`v1.34` 只保留一个明确的 RH100 生图入口：自动判断 T2I/I2I，提交后由用户决定是否下载，确认后统一保存到桌面任务文件夹。

### v1.33

`v1.33` 统一了 `文生图` 和 `图生图` 的单任务前台轮询策略，让生成完成后的下载等待更可控：

- `文生图/scripts/rh100_t2i.py` 默认轮询间隔保持为 `10` 秒，最大前台等待时间改为 `120` 秒。
- `图生图/scripts/rh100_i2i.py` 默认轮询间隔保持为 `10` 秒，最大前台等待时间改为 `120` 秒。
- 两个 skill 的 `SKILL.md` 示例命令和说明同步更新为 `--poll-seconds 10` 与 `--max-wait-seconds 120`。
- 这次只调整单任务脚本的前台等待策略，批处理脚本仍保留原有短窗口轮询规则。

简而言之，`v1.33` 让文生图和图生图在 Codex 前台生成时都按同一节奏等待：每 10 秒查一次，最多等 2 分钟，成功就下载。

### v1.32

`v1.32` 将 RH100 的两个独立生图 skill 改成中文命名，并同步优化默认使用入口：

- `RH100-T2I/` 更名为 `文生图/`，skill 名称、展示名、默认提示、命令示例路径和 API 参考标题同步改为“文生图”。
- `RH100-I2I/` 更名为 `图生图/`，skill 名称、展示名、默认提示、命令示例路径和 API 参考标题同步改为“图生图”。
- `文生图` 明确默认走快速路径：拿到提示词后直接调用脚本提交任务，只有用户显式要求等待时才轮询，只有用户要求提示词优化时才展开长分析。
- `图生图` 保留原有单任务和批处理脚本入口，下载文件名前缀改为 `tushengtu_`；`文生图` 下载文件名前缀改为 `wenshengtu_`。
- `.gitignore` 白名单同步从旧英文目录切换到中文目录，避免新目录被忽略。

简而言之，`v1.32` 让常用 RunningHub 生图能力直接以“文生图 / 图生图”出现，入口更直观，默认执行也更少绕路。
...
- 现在 `RH100-T2I` 已统一与 `RH100-I2I` 保持同一命名风格。
...
简而言之，`v1.29` 让这三个 skill 从“本地可用但未入库”变成“可版本化发布”的正式个人 skills，并把 `RH100` 相关目录命名统一了。
...
- `RH100` 新增默认批量执行节奏：先 `submit`，约 `60` 秒后做第一次短轮询，再按约 `30` 秒间隔追加最多 `2` 次短轮询；到第 `3` 次检查仍未全完成时，停止长时间前台等待，直接汇报已完成和未完成数量。
- `RH100` 明确把 `stream disconnected before completion: Upstream request failed` 视为 Codex/工具传输中断风险场景，因此默认采用“短前台、多次短查”的方式替代持续长轮询。
- `RH100` 的进度汇报格式升级为按图片顺序展示的紧凑多列表格，使用 `✅` / `❌` 让用户一眼判断哪些图片已完成、哪些仍在运行。
- `RH100` 的完成汇报改为精简样式：结果目录使用 `📁`，墙钟耗时使用 `🕒`，金额使用 `💰`，默认不再展开冗长的逐文件输出列表，也不再显示 `consumeMoney = N/A`、`consumeCoins = N/A` 和记录文件段落。
- `amazon-plus-1.0` 新增 `Short Completion Default`：生成工具已落盘时，默认立刻汇报状态、数量、输出目录、耗时和可用费用字段，不自动继续做整理、QA 总览图或图片打开预览，除非用户明确要求。
- `amazon-plus-1.0` 同时收紧“Before final delivery”触发条件，只有当用户明确要求 Amazon-ready 最终打包、QA 或整理后的交付目录时，才继续执行后续包装检查步骤。

简而言之，`v1.27` 让 RH100 和 Amazon 批量生图任务更适合在 Codex 里稳定跑完并快速汇报：前台等待更短，断流影响更小，结果展示也更贴近实际使用习惯。
