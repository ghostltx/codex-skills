...
def upload_path(path):
    url, _ = rh100.upload_file(str(path))
    return str(path), url


def submit_job(job, args, log_path):
    submit = rh100.submit_task(
...
def poll_job(job, out_dir, log_path):
    ...
    result = rh100.query_task(task_id)
...
def main():
    ...
    parser.add_argument("command", choices=["run", "submit", "poll", "status"])
    ...
    parser.add_argument("--job-file", default=r"C:\Users\Administrator\Desktop\灰色\rh100_jobs.json")
    parser.add_argument("--out-dir", default=r"C:\Users\Administrator\Desktop\灰色")
    ...
    if args.command in ("run", "submit"):
        if not args.image:
            raise SystemExit("--image is required for run/submit")
        if not args.prompt:
            raise SystemExit("--prompt or --prompt-file is required for run/submit")
